...
export const calendarService = {
  ...
  createSlot: async (startTime, endTime, type) => {
    const { data } = await api.post('/api/calendar/slots', { startTime, endTime, type });
    return data;
  },
  ...
  getAvailableSlots: async (doctorId, startDate, endDate, appointmentType) => {
    ...
    const { data } = await api.get('/api/calendar/slots', { params });
    return data;
  },

  bookSlot: async (slotId, bookingData) => {
    const { data } = await api.post('/api/calendar/slots/book', {
      slotId,
      ...bookingData,
    });
    return data;
  ...
  cancelAppointment: async (appointmentId, cancellationReason) => {
    const { data } = await api.post(`/api/calendar/appointments/${appointmentId}/cancel`, {
      cancellationReason,
    });
    return data;
  },
  ...
  rescheduleAppointment: async (appointmentId, newSlotId) => {
    const { data } = await api.post(`/api/calendar/appointments/${appointmentId}/reschedule`, {
      newSlotId,
    });
    return data;
  },
